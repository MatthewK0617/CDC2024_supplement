...
def plot_comparison(
    ...
    if no_comp:
        ...
        if obs_tag == "rbf":
            plt.suptitle(
                f"{model.regressor} Traj. Linearizations (rbf) of {model_name}",
                fontsize=20,
            )
        else:
            plt.suptitle(
                f"{model.regressor} Traj. Linearizations ({model.observables}) of {model_name}",
                fontsize=20,
            )
    # plt.show()
    path = os.path.join(os.getcwd(), os.pardir, os.pardir, os.pardir)
    path = os.path.join(path, "gen", "plots", f"{model_name}_{obs_tag}")
    print(path)
